...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:variable name="schema" select="document('drMgmt.xml')/management-information"/>

  <xsl:variable name="basetypes" select="document('basetypes.xml')" />
  ...
  <func:function name="dpfunc:property-display-name">
      ...
      <xsl:param name="objname"/>
      <xsl:param name="strParentName" select="''"/>
      <xsl:variable name="propIndex" select="dpfunc:schema-object-props($objname)"/>
      <xsl:variable name="objProp" select="$propIndex/self::property[@name=$strName]"/>
      <xsl:choose>
          <!-- sub property display -->
          <xsl:when test="$objProp/display != ''">
              <func:result select="$objProp/display"/>
          </xsl:when>
          <!-- type -->
          <xsl:when test="$propIndex/self::property[@name=$strParentName]/@type and 
              $type-index/self::type[@name=$propIndex/self::property[@name=$strParentName]/@type]/properties/property[@name=$strName]/display != ''">
              <func:result select="$type-index/self::type[@name=$propIndex/self::property[@name=$strParentName]/@type]/properties/property[@name=$strName]/display"/>
          </xsl:when>
          <!-- display the property raw -->
          <xsl:otherwise>
              <func:result select="$strName"/>
          </xsl:otherwise>
      </xsl:choose>
  </func:function>
            ...
            <xsl:choose>
              ...
              <xsl:when test="$typeNode/type/default">
                <xsl:message dp:priority="debug">
                  <xsl:text>type default for @name</xsl:text>
                </xsl:message>
  ...
  <!-- also, note that this isn't recursive; we're only processing the top-level nodes -->
  <xsl:template name="unique-config">
    <xsl:for-each select="*">
      <xsl:variable name="class" select="local-name()"/>
      <xsl:variable name="name" select="@name"/>
      <xsl:if test="not(preceding::*[local-name()=$class and @name=$name])">
        <xsl:copy-of select="."/>
      </xsl:if>
    </xsl:for-each>        
  </xsl:template>
